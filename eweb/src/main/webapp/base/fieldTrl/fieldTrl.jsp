...
<html>
<head>
    <title><%--字段翻译--%><eidea:label key="fieldTrl.title"/></title>
...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
            .config(['$routeProvider', function ($routeProvider) {
                $routeProvider
                        .when('/list', {templateUrl: '<c:url value="/base/fieldTrl/list.tpl.jsp"/>'})
                        .when('/edit', {templateUrl: '<c:url value="/base/fieldTrl/edit.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($scope, $http) {
        ...
        $scope.pageChanged = function () {
            $http.post("<c:url value="/base/fieldTrl/list"/>", $scope.queryParams)
...
//可现实分页item数量
    ...
    app.controller('editCtrl', function ($scope, $http, $routeParams) {
        ...
        if ($routeParams.id != null) {
            url = "<c:url value="/base/fieldTrl/get"/>" + "?id=" + $routeParams.id;
        }
        $http.get(url)
            .success(function (response) {
                if (response.success) {
