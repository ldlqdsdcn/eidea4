...
<html>
<head>
    <title><%--系统属性设置--%><eidea:label key="settings.title"/></title>
...
<script type="text/javascript">
    var app = angular.module('myApp', ['ngFileUpload','ngRoute', 'ui.bootstrap', 'jcs-autoValidate'])
            .config(['$routeProvider', function ($routeProvider) {
                $routeProvider
                        .when('/list', {templateUrl: '<c:url value="/base/settings/list.tpl.jsp"/>'})
    ...
    app.controller('listCtrl', function ($rootScope,$scope,$http,$window) {
        ...
        $http.post("<c:url value="/base/settings/list"/>", $scope.queryParams)
        ...
        $http.post("<c:url value="/base/settings/deletes"/>", param).success(function (data) {
        if (data.success) {
    ...
    app.controller('editCtrl', function ($routeParams,$scope, $http,$window,$timeout, Upload) {
        ...
        $scope.settingsPo = {};
        $scope.canAdd=PrivilegeService.hasPrivilege('add');
        var url = "<c:url value="/base/settings/create"/>";
        if ($routeParams.key != null) {
            url = "<c:url value="/base/settings/get"/>" + "?key=" + $routeParams.key;
        }
        $http.get(url)
                .success(function (response) {
                    if (response.success) {
                        $scope.settingsPo = response.data;
                        $scope.tableId=$scope.settingsPo.key;
                        $scope.canSave=(PrivilegeService.hasPrivilege('add')&&$scope.settingsPo.key==null)||PrivilegeService.hasPrivilege('update');
                    }
                    else {
                        bootbox.alert(response.message);
                    }
        ...
        $scope.save = function () {
            ...
            if ($scope.editForm.$valid) {
                var postUrl = '<c:url value="/base/settings/saveForUpdated"/>';
                if ($scope.settingsPo.key == null) {
                    postUrl = '<c:url value="/base/settings/saveForCreated"/>';
                }
                $http.post(postUrl, $scope.settingsPo).success(function (data) {
                    if (data.success) {
                        $scope.message = "<eidea:label key="base.save.success"/>";
                        $scope.settingsPo = data.data;
                    }
                    else {
