...
<script type="text/javascript">
    ...
    var app = angular.module('myApp',['ui.router','ui.bootstrap','jcs-autoValidate'])
        .config(['$stateProvider','$urlRouterProvider',function ($stateProvider,$urlRouterProvider) {
            $urlRouterProvider.when('/edittab','edit.edittab')
                .otherwise('/list');
            $stateProvider
                .state('list', {
                    url:'/list',
...
//可现实分页item数量
    ...
    app.controller('tabCtrl',function ($scope,$rootScope,$state) {
        $scope.showList=function () {
            ...
            $state.go('edit.tablist',{id:$rootScope.id});
        }
        $scope.showEdit=function(){
            $rootScope.show=true;
        }
    });
    app.controller('listTabCtrl', function ($scope, $http,$stateParams,$state) {
        $scope.modelList = [];
        $scope.delFlag = false;
        $scope.isLoading = true;
        $scope.canDel = PrivilegeService.hasPrivilege('delete');
...
//可现实分页item数量
        ...
        $scope.editItem=function (id) {
          $state.go('edit.editlist',{tabid:id})
        }
        $scope.createItem=function () {
            $state.go('edit.editlist',{tabid:null})
        }
    });
